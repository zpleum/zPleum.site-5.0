...
interface CategoryManagerProps {
    apiEndpoint: string; // e.g., '/api/admin/categories' or '/api/admin/certificate-categories'
    title?: string;
    description?: string;
}

export default function CategoryManager({ apiEndpoint, title = "Category Registry", description = "Manage classifications" }: CategoryManagerProps) {
    ...
    const confirmDelete = async () => {
        ...
        setError('');
        try {
            const response = await fetch(`${apiEndpoint}/${deleteId}`, {
                method: 'DELETE',
            });
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(async () => {
            try {
                // Ensure endpoint ends with /reorder if it supports it, OR assume the base endpoint + /reorder
                // Construct reorder endpoint carefully. If apiEndpoint is /api/admin/categories, reorder is /api/admin/categories/reorder
                // Check if apiEndpoint ends with slash
                const reorderUrl = apiEndpoint.endsWith('/') ? `${apiEndpoint}reorder` : `${apiEndpoint}/reorder`;
    ...
    }, [categories, loading, hasChanged, apiEndpoint]);

    return (
        <div className="relative max-w-4xl mx-auto">
            <div className="flex items-center justify-between mb-8">
                <div>
                    <h2 className="text-2xl font-black tracking-tighter uppercase mb-1">{title}</h2>
                    <p className="text-xs font-black uppercase tracking-widest opacity-30 text-[var(--foreground)]">{description}</p>
                </div>
                {isSyncing && (
                    <motion.div
                        ...
                        >
                            ...
                            <div className="relative z-10 text-center">
                                ...
                                <p className="text-[var(--foreground-muted)] text-lg font-medium mb-12 leading-relaxed">
                                    Are you certain you wish to permanently terminate this category? <span className="text-red-500 font-bold uppercase tracking-wider">Warning:</span> Associated items may lose their classification.
                                </p>
